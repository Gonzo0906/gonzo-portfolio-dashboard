...
test('configured minute feed fetches providers on the 120-second timer and updates valuation',async({page})=>{
 ...
 await page.goto('/');await expect(page.locator('#coverage')).toContainText('38/39');await expect(page.locator('#feedTime')).toContainText('2m prices');const total=await page.locator('#total').innerText();expect(requests).toBe(1);
...
test('unconfigured backend does not advertise two-minute provider fetches',async({page})=>{
 await page.route('**/feed-config.json',r=>r.fulfill({json:{priceEndpoint:null,priceRefreshSeconds:120}}));await page.goto('/');await expect(page.locator('#coverage')).toContainText('38/39');await expect(page.locator('#feedTime')).toContainText('15m snapshot');await page.locator('#feedDetails').click();await expect(page.locator('#dialogBody')).toContainText('Two-minute price backend is not connected yet');
});
