...
const result=await page.evaluate(()=>{const d=document.documentElement;const clipped=[...document.querySelectorAll('.holding b,.holding small')].filter(el=>getComputedStyle(el).display!=='none').filter(el=>{const r=document.createRange();r.selectNodeContents(el);const a=r.getBoundingClientRect(),b=el.closest('.holding').getBoundingClientRect();return a.left<b.left-1||a.right>b.right+1||a.top<b.top-1||a.bottom>b.bottom+1;}).map(el=>el.textContent);return {w:d.scrollWidth,h:d.scrollHeight,clipped};});
