...
const PRIVACY_KEY='gonzo.portfolio.concealed';
let concealed=false;
...
$('updateHoldings').onclick=()=>openHoldingsUpload({assets:[...holdings.stocks.map(h=>({...h,stock:true})),...holdings.crypto.map(h=>({...h,available:h.symbol==='PI'?holdings.metadata.PI.available:undefined}))],dialog,escape,onSave:(updates,available)=>{
 ...
 const pi=next.crypto.find(h=>h.symbol==='PI');if(!validNumber(available)||available<0||available>pi.quantity)throw Error('Available PI cannot exceed the new total PI.');next.metadata.PI.available=available;
...
$('settings').onclick=()=>{dialog('Portfolio settings',`<p>Your exact screenshot holdings are the first-launch defaults. Edits and alert rules are saved locally on this device; this is not an ongoing brokerage connection.</p><p class="note">Select any holding to update its quantity and inspect provider timestamps. No corporate-action adjustments have been applied.</p><div class="actions"><button id="export">Export backup</button><button id="import">Import backup</button><button id="reset">Restore screenshot holdings</button></div><input type="file" id="backupFile" accept="application/json" hidden><p id="backupMessage" class="note"></p><p class="note">Restoring holdings retains your alert rules. A backup includes your local quantity edits and rules.</p>`);$('export').onclick=()=>{const blob=new Blob([JSON.stringify({holdings,rules},null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='gonzo-portfolio-backup.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};$('import').onclick=()=>$('backupFile').click();$('backupFile').onchange=async e=>{try{const data=JSON.parse(await e.target.files[0].text());for(const type of ['stocks','crypto']){if(!Array.isArray(data.holdings?.[type])||data.holdings[type].length!==defaults[type].length)throw Error('Holdings list does not match');for(const orig of defaults[type]){const h=data.holdings[type].find(h=>h.symbol===orig.symbol);if(!h||h.quantity!==null&&(!validNumber(h.quantity)||h.quantity<0)||h.cost!==null&&(!validNumber(h.cost)||h.cost<0)||type==='crypto'&&h.cost!==null)throw Error('Invalid holdings');}}if(!validNumber(data.holdings.metadata?.PI?.available)||data.holdings.metadata.PI.available<0||data.holdings.metadata.PI.available>data.holdings.crypto.find(h=>h.symbol==='PI').quantity)throw Error('Invalid PI available balance');const known=[...defaults.stocks,...defaults.crypto].map(h=>h.symbol);if(!Array.isArray(data.rules)||data.rules.some(r=>!known.includes(r.symbol)||!['above','below','daily','allocation'].includes(r.type)||!validNumber(r.threshold)||r.threshold<0||!validNumber(r.cooldown)||r.cooldown<1))throw Error('Invalid alert rules');holdings=structuredClone(defaults);for(const type of ['stocks','crypto'])for(const h of holdings[type]){const other=data.holdings[type].find(x=>x.symbol===h.symbol);h.quantity=other.quantity;h.cost=other.cost;}holdings.metadata.PI.available=data.holdings.metadata.PI.available;rules=data.rules.map(r=>({...r,id:crypto.randomUUID()}));ruleState={};persist();render();$('backupMessage').textContent='Backup imported.';}catch(err){$('backupMessage').textContent='Import failed: '+err.message;}};$('reset').onclick=()=>{dialog('Restore screenshot quantities?',`<p>This replaces your quantity and cost edits with the original screenshot transcription.</p><button id="confirmReset">Restore holdings</button>`);$('confirmReset').onclick=()=>{holdings=structuredClone(defaults);persist();render();$('dialog').close();};};};
$('feedDetails').onclick=()=>{const v=valuation(holdings,market.quotes);dialog('Data coverage & freshness',`<p>${minuteConnected?'The backend is checked every 2 minutes while open. If a provider rejects a request, the last public snapshot is retained with its original quote timestamp. Stock and crypto prices update through the backend every 2 minutes. News and fallback snapshots refresh every 15 minutes.':'Two-minute price backend is not connected yet. Current prices/news refresh every 15 minutes through GitHub Actions; this page checks the published snapshot every 2 minutes.'} Scheduled runs can be delayed by GitHub. Stocks use delayed public Yahoo chart quotes; crypto uses CoinGecko aggregated prices. These are not exchange streaming quotes.</p><p>Latest fetch: ${escape(time(market.fetchedAt))}. ${escape(refreshError)}</p><p class="${v.complete?'positive':'warning'}">${v.complete?'All quantities valued.':'Partial valuation. Unresolved: '+v.missing.join(', ')+'.'} Allocation percentages remain undefined until every quantity and price is known.</p><p class="note">Daily stock change uses the previous session close; crypto daily change is rolling 24h. Total return uses screenshot average cost, excludes fees/dividends, and is separate from daily movement. Zero-cost XRPR has undefined percentage return. Crypto, AMC, and PHUN have unknown basis.</p><p class="note">${storageError?'Browser storage unavailable. Export a backup.':'Edits persist in browser local storage.'}</p>${Object.entries(market.quotes).map(([s,q])=>`<p><b>${escape(s)}</b> · ${escape(q.provider)} / ${escape(q.providerId)}<br><small>${escape(quoteStatus(q,market.fetchedAt))} · ${escape(time(q.timestamp))}${q.error?'<br>'+escape(q.error):''}</small></p>`).join('')}<p>${escape((market.issues||[]).join(' · '))}</p><p><a href="https://finance.yahoo.com/" target="_blank" rel="noopener noreferrer">Yahoo Finance</a> · <a href="https://www.coingecko.com/" target="_blank" rel="noopener noreferrer">Data provided by CoinGecko</a></p>`);};
async function refresh(){if(busy)return;busy=true;$('refresh').disabled=true;try{const response=await fetch(`data/market.json?t=${Date.now()}`,{cache:'no-store',signal:AbortSignal.timeout(20000)});if(!response.ok)throw Error(`HTTP ${response.status}`);const data=await response.json();if(data.schema!==1||!data.quotes||!Array.isArray(data.news)||!validNumber(data.fetchedAt))throw Error('Invalid market snapshot');let next=data;
 if(priceEndpoint){
  const priceResponse=await fetch(priceEndpoint,{cache:'no-store',signal:AbortSignal.timeout(25000)});
  if(!priceResponse.ok)throw Error('Minute price feed HTTP '+priceResponse.status);
  const priceData=await priceResponse.json();
  if(priceData.schema!==1||!priceData.quotes||!validNumber(priceData.fetchedAt))throw Error('Invalid minute price feed');
  const mergedQuotes={...data.quotes};for(const [symbol,quote] of Object.entries(priceData.quotes)){if(validNumber(quote.price)&&quote.price>0)mergedQuotes[symbol]=quote;}
  next={...data,...priceData,quotes:mergedQuotes,news:data.news,newsStatus:data.newsStatus,newsFetchedAt:data.fetchedAt};minuteConnected=true;
 }
 const changed=next.fetchedAt!==market.fetchedAt;market=next;refreshError='';if(changed)evaluateAlerts();render();}catch(e){refreshError=e.message;render();}finally{busy=false;$('refresh').disabled=false;}}
$('refresh').onclick=refresh;
try{const response=await fetch('holdings.json');if(!response.ok)throw Error('Cannot load initial holdings');defaults=normalize(await response.json());const stored=readState();holdings=structuredClone(defaults);if(stored?.holdings){for(const type of ['stocks','crypto'])for(const h of holdings[type]){const old=stored.holdings[type]?.find(x=>x.symbol===h.symbol);if(old&&(old.quantity===null||validNumber(old.quantity)&&old.quantity>=0)){h.quantity=old.quantity;if(type==='stocks'&&(old.cost===null||validNumber(old.cost)&&old.cost>=0))h.cost=old.cost;}}if(validNumber(stored.holdings.metadata?.PI?.available))holdings.metadata.PI.available=stored.holdings.metadata.PI.available;rules=Array.isArray(stored.rules)?stored.rules:[];history=Array.isArray(stored.history)?stored.history:[];seen=Array.isArray(stored.seen)?stored.seen:null;newsPopups=stored.newsPopups!==false;ruleState=stored.ruleState||{};notificationsReadAt=validNumber(stored.notificationsReadAt)?stored.notificationsReadAt:0;}persist();
 try{const configResponse=await fetch('feed-config.json',{cache:'no-store'});if(configResponse.ok){const config=await configResponse.json();if(config.priceEndpoint){const url=new URL(config.priceEndpoint);if(url.protocol!=='https:'||url.username||url.password)throw Error('Invalid feed URL');priceEndpoint=url.href;}}}catch(e){refreshError='Price backend configuration unavailable: '+e.message;}
 setInterval(()=>{
