...
async function news(){
 const response=await fetch('https://feeds.finance.yahoo.com/rss/2.0/headline?s=%5EGSPC,BTC-USD&region=US&lang=en-US',{signal:AbortSignal.timeout(15000)});if(!response.ok)throw Error('News provider unavailable');const xml=await response.text(),items=[...xml.matchAll(/<item>([\s\S]*?)<\/item>/g)];const field=(item,key)=>decode(item.match(new RegExp('<'+key+'(?:[^>]*)>([\\s\\S]*?)</'+key+'>'))?.[1]||'').trim();return {fetchedAt:Date.now()/1000,newsStatus:'connected',news:items.slice(0,30).map(([_,item])=>{const title=field(item,'title'),url=field(item,'link'),published=Date.parse(field(item,'pubDate'))/1000;return {id:url,title,url,source:'Yahoo Finance news feed',summary:field(item,'description').slice(0,500),published:Number.isFinite(published)?published:null,tags:['Market'],summaryType:'Publisher feed summary',linkType:'Publisher article'};}).filter(a=>a.title&&/^https:\/\//.test(a.url))};
}
