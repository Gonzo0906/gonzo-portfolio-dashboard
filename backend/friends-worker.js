...
async function news(){
 let viaGoogle=false;let response=await fetch('https://feeds.finance.yahoo.com/rss/2.0/headline?s=%5EGSPC,BTC-USD&region=US&lang=en-US',{headers:{'User-Agent':'Mozilla/5.0 GonzoPortfolio/1.0'},signal:AbortSignal.timeout(15000)});if(!response.ok){viaGoogle=true;response=await fetch('https://news.google.com/rss/search?q='+encodeURIComponent('stock market OR cryptocurrency when:7d')+'&hl=en-US&gl=US&ceid=US:en',{headers:{'User-Agent':'Mozilla/5.0 GonzoPortfolio/1.0'},signal:AbortSignal.timeout(15000)});}if(!response.ok)throw Error('News provider HTTP '+response.status);const xml=await response.text(),items=[...xml.matchAll(/<item>([\s\S]*?)<\/item>/g)];const field=(item,key)=>decode(item.match(new RegExp('<'+key+'(?:[^>]*)>([\\s\\S]*?)</'+key+'>'))?.[1]||'').trim();return {fetchedAt:Date.now()/1000,newsStatus:'connected',news:items.slice(0,30).map(([_,item])=>{const title=field(item,'title'),url=field(item,'link'),published=Date.parse(field(item,'pubDate'))/1000;return {id:url,title,url,source:viaGoogle?(field(item,'source')||'Publisher via Google News'):'Yahoo Finance news feed',summary:field(item,'description').slice(0,500),published:Number.isFinite(published)?published:null,tags:['Market'],summaryType:'Publisher feed summary',linkType:viaGoogle?'Google News link to publisher':'Publisher article'};}).filter(a=>a.title&&/^https:\/\//.test(a.url))};
}
